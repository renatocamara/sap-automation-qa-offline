...
| [RUN-WITH-AZURE-ACCESS.md](./RUN-WITH-AZURE-ACCESS.md) | **The flexible option.** If a narrow, read-only path from the jump to the Azure management APIs is acceptable, the framework runs as designed and the report includes the Azure-infrastructure checks (disk performance, SKU support, storage, HA) that are N/A when fully offline. Covers the exact endpoints, the Reader identity / service principal, and where each adjustment is made. |
| [RUN-V1.1.4.md](./RUN-V1.1.4.md) | **Delta procedure to run release v1.1.4 offline.** Verified against the v1.1.4 source: dependencies identical to v1.1.3, existing bundle reused as-is, offline tolerances still required. Includes the HA-coverage (`scs_high_availability`) note for capturing corosync/quorum/fencing. |
